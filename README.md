...
- [Observabilidad](#observabilidad)
  - [Notas de troubleshooting](#notas-de-troubleshooting)
- [Testing](#testing)
- [CI/CD](#cicd)
- [Ambientes](#ambientes)
...
gRPC (puerto `4317`). Este diseño evita pérdidas de trazas y problemas de enrutamiento que ocurren
con OTLP HTTP en las versiones recientes de Tempo.
...
- **Panel de trazas en Grafana 11.x**: para evitar crasheos de la UI y ciclos infinitos de carga al
  renderizar búsquedas múltiples, el dashboard de trazas (`dashboard.json`) usa el panel genérico
  de tipo `table` con la consulta `traceqlSearch` en vez del panel experimental de tipo `traces`
  (este último puede generar un `TypeError` en el frontend al expandir la cascada).
- **Timeouts hacia Keycloak (IPv6 vs IPv4)**: si el backend arroja `SocketTimeoutException`
  recurrentes al obtener el JWK Set de Keycloak (aun estando en la misma red de Docker), es por
  resolución de IPv6 dentro del contenedor de Java. El `Dockerfile` inyecta
  `JAVA_OPTS: "-Djava.net.preferIPv4Stack=true"` desde `docker-compose.yml` para forzar IPv4 en la
  red local.
