...
ALTER TABLE products ADD CONSTRAINT chk_products_sku_not_blank CHECK (length(btrim(sku)) > 0);
ALTER TABLE products ADD CONSTRAINT chk_products_name_not_blank CHECK (length(btrim(name)) > 0);
...
ALTER TABLE stock_movements ADD CONSTRAINT chk_stock_movements_user_id_not_blank CHECK (length(btrim(user_id)) > 0);
